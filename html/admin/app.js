(function () {
    ...
    function players() { var q = state.search.toLowerCase(); return (state.data.players || []).filter(function (p) { if (!q) return true; return [p.name, p.character, p.accountId, p.characterId, p.serial, p.adminRole].join(' ').toLowerCase().indexOf(q) !== -1; }); }
    ...
    function renderStatePanels() {
        ...
        $('#staffState').innerHTML = staff.length ? staff.map(function (s) { return row(s.character || s.name, (s.adminRole || 'Staff') + ' | lvl ' + (s.adminLevel || 0)); }).join('') : '<div class="empty-state">Brak staff online.</div>';
    }
