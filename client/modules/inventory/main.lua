...
local function categoryLabel(id)
    for _, category in ipairs(Inv.categories or {}) do
        if tostring(category.id or "") == tostring(id) then
            return tostring(category.label or fallbackCategories[id] or id)
        end
    end
...
local function filteredItems()
    local list = {}
    for _, item in ipairs(Inv.items or {}) do
        if Inv.category == "all" or item.category == Inv.category then
            list[#list + 1] = item
        end
    end
    return list
...
local function drawWeightBar(x, y, w, h)
    ...
    local fill = ratio > 0.92 and color("danger", 235) or color("accent", 220)
    dxDrawRectangle(x, y, w, h, color("barBack", 165), true)
    dxDrawRectangle(x, y, math.max(2, w * ratio), h, fill, true)
    dxDrawRectangle(x, y + h - 1, w, 1, tocolor(0, 0, 0, 160), true)
end

local function drawCategories(scale)
    local x = Inv.x + 18 * scale
    local y = Inv.y + 70 * scale
    local h = 25 * scale
...
local function drawList(scale, list)
    ...
    for row = 1, rows do
        ...
        local selected = index == Inv.selected
        local fill = selected and color("rowActive", 210) or ((row % 2 == 0) and color("rowAlt", 92) or color("row", 75))
        dxDrawRectangle(x + 1, rowY, w - 2, Inv.rowH - 1, fill, true)

        if item then
            local quality = tonumber(item.quality) or 100
            local qColor = quality <= 25 and color("danger", 230) or color("text", 230)
    ...
    if #list == 0 then
        shadowText("Brak przedmiotow w tej kategorii.", x, y + headerH + 40 * scale, x + w, y + headerH + 80 * scale, color("muted", 230), 0.82 * scale, "default-bold", "center", "center")
    end

    return x, y, w, headerH, rows
end

local function drawDetails(scale, item)
    local x = Inv.x + Inv.w * 0.62
    local y = Inv.y + 108 * scale
    local w = Inv.w - (x - Inv.x) - 18 * scale
    local h = Inv.h - 178 * scale

    panelRect(x, y, w, h, color("panel", 176), color("line", 165))
    if not item then
        shadowText("Wybierz przedmiot", x + 18 * scale, y + 26 * scale, x + w - 18 * scale, y + 70 * scale, color("muted", 230), 0.9 * scale, "default-bold", "center", "center")
        return
    end
    ...
    local yy = y + 82 * scale
    local rows = {
        { "Ilosc", "x" .. tostring(item.quantity or 1) },
...
local function drawFooter(scale)
    local y = Inv.y + Inv.h - 48 * scale
    local text = "I / ESC - zamknij    TAB - kategoria    ENTER - uzyj    BACKSPACE - wyrzuc 1    DEL - wyrzuc stos"
    shadowText(text, Inv.x + 18 * scale, y, Inv.x + Inv.w - 18 * scale, y + 24 * scale, color("muted", 225), 0.68 * scale, "default-bold", "center")
end
...
local function renderInventory()
    ...
    local weightX = Inv.x + Inv.w - 260 * scale
    local weightY = Inv.y + 24 * scale
...
local function requestSync()
    triggerServerEvent("HeavyRPG:Inventory:request", resourceRoot)
end
...
local function setVisible(state)
    ...
    if state then
        ...
        if not clickAttached then
            addEventHandler("onClientClick", root, Inv.handleClick)
            clickAttached = true
        end
    else
        removeEventHandler("onClientRender", root, renderInventory)
        if clickAttached then
            removeEventHandler("onClientClick", root, Inv.handleClick)
            clickAttached = false
        end
    end
end
...
function Inv.handleClick(button, state, absX, absY)
    ...
    local scale = uiScale()
    local list = filteredItems()
    local listX = Inv.x + 18 * scale
    local listY = Inv.y + 108 * scale
    local listW = Inv.w * 0.58
    local headerH = 24 * scale
    local rows = maxVisibleRows()

    if absX >= listX and absX <= listX + listW and absY >= listY + headerH and absY <= listY + headerH + rows * Inv.rowH then
        local row = math.floor((absY - listY - headerH) / Inv.rowH) + 1
        local index = Inv.scroll + row
...
local function nextCategory()
    local cats = categoryOrder()
    local current = 1
    for index, id in ipairs(cats) do
        if id == Inv.category then current = index break end
    end
    current = current + 1
    if current > #cats then current = 1 end
    Inv.category = cats[current]
    Inv.selected = 1
    Inv.scroll = 0
end
...
local function selectedAction(action)
    ...
    if action == "use" and item.usable then
        triggerServerEvent("HeavyRPG:Inventory:use", resourceRoot, item.uid)
    elseif action == "drop_one" then
        triggerServerEvent("HeavyRPG:Inventory:drop", resourceRoot, item.uid, 1)
    elseif action == "drop_stack" then
        triggerServerEvent("HeavyRPG:Inventory:drop", resourceRoot, item.uid, item.quantity or 1)
    end
end
...
local function handleKey(button, press)
    ...
    if button == tostring(cfg().key or "i") then
        if canToggle() then setVisible(not Inv.visible) end
        cancelEvent()
        return
    end
    ...
    elseif button == "backspace" then
        selectedAction("drop_one")
        cancelEvent()
    elseif button == "delete" then
        selectedAction("drop_stack")
        cancelEvent()
    end
end
...
addEventHandler("HeavyRPG:Inventory:open", resourceRoot, function()
    setVisible(true)
end)
